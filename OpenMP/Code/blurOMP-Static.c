...
int main (int argc, const char * argv[]) {
    ...
    int thread_count = atoi(argv[1]); // string to int
    int chunk_size = atoi(argv[2]); // string to int
    ...
    for(k=0;k<nblurs;k++){
        #pragma omp parallel shared(R, G, B, Rnew, Gnew, Bnew, chunk_size) private(row, col) num_threads(thread_count)
        #pragma omp for collapse(2) schedule(static, chunk_size) nowait
